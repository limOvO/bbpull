...
| **文件列整列空白** | `theme.draw_icon` 把 `QColor` 傳進只接受十六進位字串的 `blend()`，`document` 分支拋 `AttributeError: 'QColor' object has no attribute 'lstrip'`。**Qt 會吞掉 `paint()` 裡的例外**，所以程式照跑但什麼都沒畫；而圖示畫在標題、描述與按鈕之前，於是整列文件都空白（真實課程有 46 個文件）。截圖用的課程根目錄剛好沒有純文件，所以看不出來 | `blend()` 改為接受 `QColor`（鴨子型別，維持 toolkit-free）；新增直接呼叫 `paint()` 的測試涵蓋每一種項目類型——直接呼叫會讓例外浮上來，不再被 Qt 吃掉 |
| **登入按鈕的延遲匯入路徑寫錯** | 連線對話框的送出處理常式寫 `from ..gui.wizard import ...`，但模組其實是 `bbpull.wizard`（沒有 `bbpull.gui.wizard`）。按下「登入」得到 `No module named 'bbpull.gui.wizard'`。因為是**函式內的延遲匯入**，模組載入、單元測試、介面測試全都碰不到 | 修正路徑；新增 `tests/test_imports.py` 走訪所有模組的 AST，解析**每一個**匯入（含延遲匯入），並與 spec 的 hidden imports 對帳 |
| **效能測試在量機器而不是量程式** | 捲動測試用絕對門檻（「3,000 列 < 3 秒」），結果隔壁的 Qt 測試改變行程狀態後就變成 11.5 秒而失敗——它測到的是環境雜訊 | 改成量**比例**：300 列與 3,000 列的每步成本（實測 17.17 ms vs 16.67 ms，10 倍列數幾乎相同），這才是「虛擬化」真正的主張 |
